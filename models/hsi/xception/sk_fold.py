...
for i in range(5):
  ...
  df_tr = pd.read_csv(tr_path).iloc[:,[1,2]]
  tr_dataset = MyDataset(df_tr, img_transforms)

  df_val = pd.read_csv(val_path).iloc[:,[1,2]]
  val_dataset = MyDataset(df_val, img_transforms)

  df_tst = pd.read_csv(tst_path).iloc[:,[1,2]]
...
pickle.dump(test_results, open(os.path.join('models/hsi/xception', 'sk_fold_test_results.pkl'), 'wb'))
